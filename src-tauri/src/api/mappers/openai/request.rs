...
pub fn transform_openai_request(
    ...
    let config = crate::api::mappers::common_utils::resolve_request_config(
        &request.model,
        &mapped_model_lower,
        &tools_val,
    ...
    // [FIX] is_claude_thinking 应该同时检查 claude 字样和 -thinking 后缀
    let is_claude_thinking = mapped_model_lower.contains("claude") && mapped_model_lower.ends_with("-thinking");
                ...
                for (_index, tc) in tool_calls.iter().enumerate() {
                    ...
                    */
                    ...
                    if let Some(original_schema) = tool_name_to_schema.get(&tc.function.name) {
                        crate::api::common::json_schema::fix_tool_call_args(&mut args, original_schema);
                    }
    ...
    if actual_include_thinking {
        ...
        let tb_config = crate::api::config::get_thinking_budget_config();
        let user_budget: i64 = user_thinking_budget.map(|b| b as i64).unwrap_or(32000);
        
        let budget = match tb_config.mode {
            crate::api::config::ThinkingBudgetMode::Passthrough => {
            ...
            crate::api::config::ThinkingBudgetMode::Custom => {
                // 自定义模式：使用全局配置的固定值
            ...
            crate::api::config::ThinkingBudgetMode::Auto => {
    ...
    crate::api::mappers::common_utils::deep_clean_undefined(&mut inner_request);
    ...
    if let Some(tools) = &request.tools {
        ...
        for tool in tools.iter() {
            ...
            if let Some(params) = gemini_func.get_mut("parameters") {
                ...
                crate::api::common::json_schema::clean_json_schema(params);
    ...
    **Proactiveness**";
    ...
    if config.inject_google_search {
        crate::api::mappers::common_utils::inject_google_search_tool(&mut inner_request);
    }
